...
// =====================  Paki Update nalang Dito na Babasahin na rin yung naka save na File ===
...
// ... (Dito yung mga existing definitions mo gaya ng let player, etc.)

// 1. ADD THIS: Function para i-rebuild ang objects mula sa JSON
function rebuildWorldFromSave() {
    const hasData = loadGameData(); // Galing sa saveSystem.js
    if (!hasData) return;

    // A. Rebuild Roads (kung may laman ang nakalatagNaMgaDaan set)
    nakalatagNaMgaDaan.forEach(posKey => {
        const [x, z] = posKey.split(',').map(Number);
        // Tawagin ang function mo na nag-a-add ng road mesh sa scene
        // Assumption: may function kang createRoadMesh(x, z)
        createRoadMesh(x, z); 
    });

    // B. Rebuild Buildings
    lahatNgBuildings.forEach(b => {
        const geom = new THREE.BoxGeometry(3, 3, 3);
        const mat = new THREE.MeshStandardMaterial({ color: b.color, roughness: 0.5 });
        const building = new THREE.Mesh(geom, mat);
        building.position.set(b.x, b.y, b.z);
        scene.add(building);
    });
    
    console.log("World rebuilt from save file!");
}

// 2. I-update ang pag-start ng Engine
// Hanapin ang part kung saan mo tinatawag ang `animate()`
// Siguraduhin na ganito ang flow:
window.addEventListener('load', () => {
    // Isang beses lang tatawagin ang load
    rebuildWorldFromSave();
    
    // Ngayon ay start na ang animation
    animate();
    isEngineLoaded = true; 
});

// 3. Sagot sa tanong mo tungkol sa `let time = 0`:
// Ang `time` variable ay ginagamit para sa "Math.cos(time)" at "Math.sin(time)". 
// Ito ang nagpapatakbo ng "Sun Orbit" (yung paikot-ikot na araw sa langit).
// Kung i-remove mo ito, titigil ang araw sa iisang pwesto.